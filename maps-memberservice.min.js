/*
 * datawrapper / vis / maps v1.0.0
 * generated on 2016-05-26T22:47:34+02:00
 */
/*! datawrapper - v1.10.3 */
...
window.cancelAnimationFrame=function(id){clearTimeout(id);};}());(function(){dw.visualization.register('maps','raphael-chart',{render:function(el){var me=this;me.setRoot(el);_.templateSettings={interpolate:/\{\{(.+?)\}\}/g,evaluate:/\{%(.+?)%\}/g};$.fn.qtip.defaults.style.classes='ui-tooltip-datawrapper';if(me.map&&me.get('map')==me.__lastSVG){me.updateMap();me.renderingComplete();}else{if(me.map){me._reset();}else{me.notifications=[];}
if(me.get('map')!==undefined&&me.get('map-path')!==undefined){me.loadMap(el);}}},reset:function(){},_reset:function(){var me=this;me._localized_labels=undefined;me.map_meta=undefined;me.map.clear();$('#chart').html('').off('click').off('mousemove').off('mouseenter').off('mouseover');$('.chart .filter-ui').remove();$('.chart .legend').remove();},getSVG:function(){var me=this;return"http://d3n8a8pro7vhmx.cloudfront.net/themes/5253646978c2720367000002/attachments/original/1464302340/worldmap.svg?1464302340";},loadMap:function(el){var me=this,c=me.__initCanvas({}),$map=$('<div id="map"></div>').html('').appendTo(el),map_path=me.get('map-path');function getMapMeta(){return $.getJSON('assets/'+map_path+"/map.json").done(function(res){me.map_meta=res;});}
function getLabelJSON(){var mapOpt=_.find(me.meta.options.map.options,function(m){return m.path==map_path;});if(mapOpt&&mapOpt.has_locale){return $.getJSON('assets/'+map_path+'/locale.json').done(function(res){me._all_locales=res;var loc=me.chart().locale().slice(0,2);if(res[loc])me._localized_labels=res[loc];});}
return false;}
